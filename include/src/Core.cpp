...
bool Core::PushTask(StructPtr& new_task)
{
    //구조체의 point를 mainTaskQueue에 넣어준다
    lock_guard<mutex> lock(queueMutex_);
    mainTaskQueue_.push(new_task);
    //TaskManager의 Dista
    return true;
}

bool Core::PopTask(StructPtr& popped_task)
{
    //구조체의 point을 pop해서 popped_task에 저장
    lock_guard<mutex> lock(queueMutex_);
    if (mainTaskQueue_.empty())
        return false;

    popped_task = mainTaskQueue_.front();
    mainTaskQueue_.pop();
    tempTaskQueue_.push(popped_task);
    return true;
}