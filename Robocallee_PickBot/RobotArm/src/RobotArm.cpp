...
speed_t RobotArm::getBaudrateConstant(int baudrate)
{
     switch (baudrate)
     {
...
bool RA::RobotArm::write(const vector<uint8_t>& data)
{
...
std::future<std::vector<uint8_t>> RobotArm::AsyncWrite(const std::vector<uint8_t>& data)
{
...
void RobotArm::handlePacket(const std::vector<uint8_t>& packet)
{
...
void RobotArm::floatToUInt16(float data, uint8_t & high, uint8_t & low)
{
...
float RobotArm::uInt16ToFloat(uint8_t high, uint8_t low)
{
...
bool RobotArm::Connet(const std::string& port, const int baudrate)
{
    ...
    reader_ = std::thread(&RobotArm::ReadLoop,this);
    parser_ = std::thread(&RobotArm::ParseLoop,this);

    return true;
}

bool RobotArm::DisConnet()
{
    if(fd_ > 0)
    {
         // 버퍼 정리
...
bool RobotArm::PowerOn()
{
    vector<uint8_t> cmd = { 0xFE, 0xFE, 0x02, 0x10, 0xFA };
    return write(cmd);
}

bool RobotArm::PowerOff()
{
    vector<uint8_t> cmd = { 0xFE, 0xFE, 0x02, 0x11, 0xFA };
    return write(cmd);
}

bool RobotArm::GetAngles(vector<float>& angles)
{
    try
    {
...
bool RobotArm::SetAngles(const vector<float>& angles, const uint8_t speed)
{
...
bool RobotArm::SetAngle(const uint8_t joint, const float angle, const uint8_t speed)
{
...
bool RobotArm::SetCoords(const std::vector<float>& Coords,const uint8_t speed, const uint8_t Mode)
{
    ...
    {
        uint8_t low , high;
        floatToUInt16(Coords[i] *10 ,high,low);
    ...
    {
        uint8_t low , high;
        floatToUInt16(Coords[i] *100 ,high,low);
...
bool RobotArm::GetCoords(std::vector<float>& Coords)
{
       try
    {
        ...
        if(future.wait_for(std::chrono::milliseconds(timeout_)) == std::future_status::ready)
        {
            ...
            for(int i = Dataframe; i < Dataframe + 6 ; i+=2)
            {
                ...
                Coords.push_back(Coordi/10.0);
            }

            for(int i = Dataframe + 6 ; i < response.size()-1 ; i+=2)
            {
                float angle = uInt16ToFloat(response[i],response[i+1]);
                Coords.push_back(angle/100.0);
            }
        }
        else
        {
...
bool RobotArm::SetEndCooriSys(uint8_t tool)
{
...
bool RobotArm::SetGrippeerState(const Gripper girpper, const uint8_t speed)
{
    std::vector<uint8_t> cmd = {0xFE, 0xFE, 0X03,0X89};
...
bool RobotArm::GetGripperValue(int& val)
{
    try
    {
        vector<uint8_t> cmd = {0xFE, 0xFE, 0x02, 0x65, 0xFA};
        ...
        if(future.wait_for(std::chrono::milliseconds(timeout_)) == std::future_status::ready)
        {
            ...
            val = static_cast<int>(response[0]);
        }
        else
        {
            std::cout << "응답 타임아웃!" << std::endl;
